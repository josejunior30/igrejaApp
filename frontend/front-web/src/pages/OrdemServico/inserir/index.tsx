  ...
  const handleSubmit = async () => {
    ...
    const ordemParaSalvar: OrdemServico = {
      ...
      servicos: servicos.map((servico) => ({
        ...
        materialObra: servico.materialObra.map((material) => ({
          nome: material.nome,
          checkInConfirmado: material.checkInConfirmado,
        })),
      })),
    };

    try {
      const response = await insertOrdem(ordemParaSalvar);
              ...
              {servicos.map((servico, servicoIndex) => (
                <div key={servicoIndex} className="servico-item mb-4">
                  ...
                  <div className="d-flex ">
                    <button
                      onClick={() => openModal(servicoIndex)}
