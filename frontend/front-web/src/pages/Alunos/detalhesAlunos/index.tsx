...
const DetalhesAlunos = () => {
  ...
  const generatePDF = () => {
    ...
    const dataNascimento = alunosDTO.dataNascimento ? new Date(alunosDTO.dataNascimento).toLocaleDateString() : "-------";
    const telefone = alunosDTO.telefone ?? "-------";
    const email = alunosDTO.email ?? "-------";
    const curso = alunosDTO.projetos ? alunosDTO.projetos.nome : "-------";
    const perguntaText = alunosDTO.pergunta ? `Sim, ${alunosDTO.pergunta}` : "Não";
...
fornecer o apoio necessário para sua participação. `, 14, 150);
    doc.text(`Eu autorizo o aluno(a) a voltar para casa sozinho(a)? (    ) NÃO        (    ) SIM `, 14, 200);
    doc.text(`__________________________________________________________Data:___/___/_____

                                    Assinatura do Responsável`, 14, 240);
